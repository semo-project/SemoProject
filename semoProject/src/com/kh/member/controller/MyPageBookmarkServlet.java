...
import com.kh.bookmark.model.service.BookmarkService;
...
public class MyPageBookmarkServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new BookmarkService().countList(loginUser.getMemberNo());
		...
		/*
		 * SELECT * FROM (SELECT ROWNUM RNUM, A.* FROM (SELECT BID, CNAME, BTITLE,
		 * USER_NAME, BCOUNT, CREATE_DATE FROM BOARD B JOIN CATEGORY USING(CID) JOIN
		 * MEMBER ON(BWRITER=USER_NO) WHERE BTYPE=1 AND B.STATUS='Y' ORDER BY BID DESC)
		 * A) WHERE RNUM BETWEEN ? AND ?
		 */

		ArrayList<Bookmark> list = new BookmarkService().selectList(pi, loginUser.getMemberNo());
