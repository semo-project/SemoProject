<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.ArrayList, com.kh.epLike.model.vo.EpLike, com.kh.board.model.vo.*" %>
<% 
	ArrayList<EpLike> list = (ArrayList<EpLike>)request.getAttribute("list");
    ...
    <div class="row">
      ...
      <div class="col-lg-9 mb-4">
	         ...
	         <% } else { %>
	          <% for(EpLike e : list) { %>
	            <div class="card h-100">
	              <a href="<%= contextPath %>/work.wo?wNo=<%= e.getEpisodeNo() %>"><img src="<%= e.getThumbnailModify() %>" title="<%= e.getWorkTitle() %>" width="100%" height="71%"></a>
	              <div class="card-body">
	                <h4 class="card-title">
	                  <a href="<%= contextPath %>/work.wo?wNo=<%= e.getEpisodeNo() %>"><%= e.getWorkTitle() %></a>
	                </h4>
	              </div>
	            </div>
	          <% } %>
		...
		<div class="pagingArea" align="center">
			<!-- 맨 처음으로 (<<) -->
			<button class="btn btn-simple" onclick="location.href='<%=contextPath%>/like.me';"> &lt;&lt; </button>
			
			<!-- 이전페이지(<) -->
			<%if(currentPage == 1){ %>
			<button class="btn btn-simple" disabled> &lt; </button>
			<%}else{ %>
			<button class="btn btn-simple" onclick="location.href='<%=contextPath%>/like.me?currentPage=<%=currentPage-1%>';"> &lt; </button>
			<%} %>
			...
			<%for(int p=startPage; p<=endPage; p++){ %>
				...
				<%}else{ %>
				<button class="btn btn-simple" onclick="location.href='<%=contextPath%>/like.me?currentPage=<%=p%>';"> <%= p %> </button>
				<%} %>
			...
			<!-- 맨 마지막으로 (>>) -->
			<button class="btn btn-simple" onclick="location.href='<%=contextPath%>/like.me?currentPage=<%=maxPage%>'"> &gt;&gt; </button>
		</div>
      </div>
    </div>
    <!-- /.row -->
