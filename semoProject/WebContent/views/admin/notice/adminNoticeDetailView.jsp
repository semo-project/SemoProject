...
<body class="sb-nav-fixed">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                        	<label style="font-weight:bold;"><%=n.getNoticeTitle() %></label>
                        	<input type="hidden" name="hiddenNo" value="<%=n.getNoticeNo()%>">
                        	<label style="float:right;">
                        		작성일자 ( <%=n.getNoticeDate() %> )
                        	</label>
                        </div>
                        <div class="card-body">

                            <div class="table-responsive">
                                <textarea class="diyDiv mb-4" style="width:100%; height:450px; resize:none;" readonly><%=n.getNoticeContent() %>
								</textarea>
								<div>
									<span>
    ...
    <script>
    	function goList() {
    		location.href = "<%=contextPath%>/alist.no?pageId=7";
    	}
    	function update() {
    		var no = $("input[name=hiddenNo]").val();
    		location.href = "<%=contextPath%>/adUpdateForm.no?pageId=7&&no=" + no;
    	}
    	$(function() {
    		$("#deleteBtn").click(function() {
    			var result = confirm('삭제 시 복구가 되지 않습니다.\n삭제하시겠습니까?');
    			if(result) {
	    			var no = <%=n.getNoticeNo()%>;
    				location.href = "<%=contextPath%>/adDelete.no?pageId=7&&no=" + no;	
    			}	
    		});
    	});
    </script>
